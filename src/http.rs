...
use crate::{
    BrpBatch, BrpError, BrpMessage, BrpRequest, BrpResponse, BrpResult, BrpSender, RemoteMethods,
    error_codes,
};
...
impl From<&HttpServerConfig> for ServerObject {
    fn from(value: &HttpServerConfig) -> Self {
        ...
        let headers_pairs = value
            ...
            .collect::<Vec<_>>();
        let extensions: HashMap<Cow<'static, str>, serde_json::Value> = if !headers_pairs.is_empty()
        {
            if let Ok(value) = serde_json::to_value(headers_pairs) {
                [("headers".into(), value)].into()
            } else {
                HashMap::new()
            }
        } else {
